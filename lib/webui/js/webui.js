...
var WebUiClient = function() {
    ...
    this.socket.on('error', function(id, error) {
        ...
        $(row).html('<div class="two columns source">' + source + '</div><div class="eight columns"><strong>Error: </strong>' + error + '</div></div>')
    });
    ...
    this.socket.on('syncing', function(id) {
        ...
        var source = $('.source', row).html();
        $(row).html('<div class="two columns source">' + source + '</div><div class="eight columns"><strong>syncing</strong></div></div>')
    });
